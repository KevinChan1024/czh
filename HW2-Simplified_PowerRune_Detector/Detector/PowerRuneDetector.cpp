...
bool PowerRuneDetector::operator()(const Mat &inputImg, Point2f& target, Mat& outImg, const bool showResult) {
    ...
    if (!hierarchy.empty())
	{
		for (int i = 0; i < nTempl; i++)
			...
			{
				Mat inputImg_r;
				RotatedRect rRect = minAreaRect(contours[j]);//寻找最小的外接轮廓矩形
				drawRotatedRect(outImg, rRect, Scalar(0, 0, 255));//画轮廓
				inputImg_r = perspectiveTransform(rRect, inputImg);//矩形旋转
				float matchval = getTemplateMatchVal(inputImg_r, templArray[i], rRect.center);//模板匹配
				if (matchval < matchValThresh)
				{
					isDetected = true;
				}
				else continue;
